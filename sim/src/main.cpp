...
int main(int argc, char** argv) {
    ...
    perf_monitor.add_perf_counter({
            "IPC", &sim_base.commit_num,
            &sim_base.cycle_num
        }
    );
    perf_monitor.add_perf_counter({
            "CPI", &sim_base.cycle_num,
            &sim_base.commit_num
        }
    );
        ...
        sim_base.add_after_clk_rise_task({
            [&] {
                ...
                if (top->io_difftest_valid) {
                    ...
                    if (has_exception) {
                        itrace->riscv_disasm(top->io_difftest_bits_inst_info_0_inst, top->io_difftest_bits_last_pc);
    ...
    console->info(
        ...
        sim_base.cycle_num, duration.count(), static_cast<double>(sim_base.commit_num) / (duration.count() + 1)
    );
