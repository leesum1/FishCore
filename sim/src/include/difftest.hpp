...
public:
    ...
    bool check_gprs(const read_gpr_fuc& dut_gpr, const read_gpr_fuc& ref_gpr) const;
...
inline bool DiffTest::check_pc(uint64_t ref_pc, uint64_t dut_pc) const {
    ...
    diff_trace->info("ref pc: 0x{:016x}, dut pc: 0x{:016x}", ref_pc,
                     dut_pc);
...
inline bool DiffTest::check_csrs(const read_csr_fuc& dut_csr) {
    ...
    for (auto idx = 0; idx < need_csr.size(); idx++) {
        if (dut_csrs[idx] != ref_csrs[idx]) {
            mismatch = true;
            logger->info(
                "csr {:04x},{:s} mismatch, ref value: 0x{:016x}, dut value: 0x{:016x}",
