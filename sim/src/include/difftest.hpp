...
public:
  ...
  void load_file(const char *file_name) { ::load_file(rv64emu_ref, file_name); }

  void step(uint64_t steps) { ::step(rv64emu_ref, steps); }

  uint64_t get_pc() { return ::get_pc(rv64emu_ref); }

  void set_pc(uint64_t pc) { ::set_pc(rv64emu_ref, pc); }

  uint64_t get_reg(uintptr_t idx) { return ::get_reg(rv64emu_ref, idx); }
  ...
  void set_csr(uint64_t idx, uint64_t val) { ::set_csr(rv64emu_ref, idx, val); }

  using read_gpr_fuc = std::function<uint64_t(size_t idx)>;
  using read_csr_fuc = std::function<uint64_t(size_t idx)>;

  bool check_gprs(const read_gpr_fuc &dut_gpr, read_gpr_fuc ref_gpr,
                  bool debug_en);
...
DiffTest::DiffTest(uint64_t boot_pc, uintptr_t memory_size,
                   uintptr_t memory_base) {
  rv64emu_ref = create_rv64emu("rv64imac", "bare", boot_pc, memory_size,
                               memory_base, 0, false, true);
}

DiffTest::~DiffTest() { destroy_rv64emu(rv64emu_ref); }

bool DiffTest::check_gprs(const DiffTest::read_gpr_fuc &dut_gpr,
                          const DiffTest::read_gpr_fuc ref_gpr,
                          bool debug_en = false) {
...
bool DiffTest::check_csrs(const DiffTest::read_csr_fuc &dut_csr,
                          bool debug_en = false) {
...
void DiffTest::ref_skip(const DiffTest::read_gpr_fuc &dut_gpr,
                        uint64_t dut_pc) {
  set_pc(dut_pc);
  for (int idx : std::views::iota(0, 32)) {
