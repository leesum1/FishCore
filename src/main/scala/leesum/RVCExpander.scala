...
import leesum.ToAugmented._

// From rocket-chip RVC
...
class RVCExpander(use_rvc: Boolean, useAddiForMv: Boolean = false)
    extends Module {
  ...
  if (use_rvc) {
    ...
    io.out := new RVCDecoder(io.in, 64, useAddiForMv).decode
  } else {
    io.rvc := false.B
    io.out := new RVCDecoder(io.in, 64, useAddiForMv).passthrough
  }
}

object RVCExpander {
  def apply(in: UInt, use_rvc: Boolean) = {
    require(in.getWidth == 16)
    val expander = Module(new RVCExpander(use_rvc, true))
    expander.io.in := Cat(0.U(16.W), in)
    expander.io.out.bits
  }
}

class DummyRVCExpander extends Module {
  val io = IO(new Bundle {
    val in = Input(UInt(32.W))
    val out = Output(new ExpandedInstruction)
    val rvc = Output(Bool())
  ...
  io.rvc := io.in(1, 0) =/= 3.U
  io.out := new RVCDecoder(io.in, 64, true).decode
}

object Gen_RVCExpander_verilog extends App {
  GenVerilogHelper(new DummyRVCExpander)
}
